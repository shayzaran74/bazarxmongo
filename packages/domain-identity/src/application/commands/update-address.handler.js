"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.UpdateAddressHandler = void 0;
const cqrs_1 = require("@nestjs/cqrs");
const common_1 = require("@nestjs/common");
const update_address_command_1 = require("./update-address.command");
const shared_core_1 = require("@barterborsa/shared-core");
const address_response_dto_1 = require("../dtos/address-response.dto");
let UpdateAddressHandler = class UpdateAddressHandler {
    addressRepository;
    constructor(addressRepository) {
        this.addressRepository = addressRepository;
    }
    async execute(command) {
        const { userId, addressId, dto } = command;
        const address = await this.addressRepository.findById(addressId);
        if (!address || address.userId !== userId) {
            return (0, shared_core_1.Err)(new shared_core_1.NotFoundException('Adres bulunamadı.'));
        }
        address.update(dto);
        if (dto.isDefault) {
            await this.addressRepository.setDefault(addressId, userId);
        }
        await this.addressRepository.update(address);
        return (0, shared_core_1.Ok)(address_response_dto_1.AddressResponseDto.fromEntity(address));
    }
};
exports.UpdateAddressHandler = UpdateAddressHandler;
exports.UpdateAddressHandler = UpdateAddressHandler = __decorate([
    (0, cqrs_1.CommandHandler)(update_address_command_1.UpdateAddressCommand),
    __param(0, (0, common_1.Inject)('IUserAddressRepository')),
    __metadata("design:paramtypes", [Object])
], UpdateAddressHandler);
//# sourceMappingURL=update-address.handler.js.map