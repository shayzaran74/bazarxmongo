      ...
      <div class="text-center mb-10">
        ...
        >
          ...
          >
            <a
              v-if="ann.linkUrl"
              :href="ann.linkUrl.startsWith('http') ? ann.linkUrl : (ann.linkUrl.startsWith('/') ? runtimeConfig.public.apiBase + ann.linkUrl : ann.linkUrl)"
              target="_blank"
          ...
          <div class="p-8 sm:p-12 flex-1">
            ...
            >
              <div class="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
                <div>
                  <h2 class="text-3xl font-bold text-neutral-900 mb-6 font-display">
                    BazarX'te Satış Yapmanın Ayrıcalıklarını Keşfedin
                  </h2>
                  <div class="space-y-6">
                    <div
                ...
                <div class="bg-primary-50 rounded-3xl p-8 border border-primary-100 flex flex-col items-center text-center">
                  <div class="w-32 h-32 bg-white rounded-3xl shadow-xl flex items-center justify-center text-5xl mb-6">
                    🛍️
                  </div>
                  <p class="text-primary-900 font-medium italic text-lg leading-relaxed">
                    "BazarX ile satışlarım %300 arttı! İşimi dijital dünyada bu kadar kolay büyütebileceğimi hayal etmezdim."
                  </p>
                  <p class="text-primary-600 font-bold mt-4 uppercase tracking-widest text-xs">
            ...
            >
              <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                ...
                <div class="space-y-6">
                  ...
                  <div class="max-h-60 overflow-y-auto pr-4 space-y-2 custom-scrollbar">
                    <label
            ...
            >
              ...
              <div class="space-y-4">
                ...
                <div v-if="legalDocs.length === 0" class="text-center py-8 text-neutral-500 italic">
            ...
            >
              ...
              <h2 class="text-3xl font-bold text-neutral-900 font-display">
...
import { useNuxtApp, navigateTo, useRuntimeConfig, useHead } from '#imports'
...
const benefits = [
  { icon: '🚀', title: 'Hızlı Başlangıç', desc: 'Dakikalar içinde mağazanızı açın ve ürünlerinizi milyonlara ulaştırın.' },
  { icon: '🏠', title: 'Esnek Yönetim', desc: 'İşinizi dilediğiniz yerden, dilediğiniz zaman BazarX paneliyle yönetin.' },
  { icon: '💳', title: 'Güvenli Tahsilat', desc: 'Ödemeleriniz koruma altında, haftalık periyotlarla hesabınızda.' },
  { icon: '📊', title: 'Gelişmiş Analitik', desc: 'Detaylı raporlarla satışlarınızı ve stoklarınızı anlık takip edin.' }
]
...
const fetchLegalDocs = async () => {
  try {
    ...
    if (response.success) {
      // API'den gelen tüm aktif dökümanları kullanıyoruz
...
const nextStep = () => {
  if (currentStep.value === 2 && (!form.value.businessName || !form.value.businessType)) {
    nuxtApp.$toast.warning('Lütfen zorunlu alanları doldurun')
    return
  }
  if (currentStep.value === 3 && (!form.value.phone || !form.value.email || !form.value.address)) {
    nuxtApp.$toast.warning('İletişim ve adres bilgileri zorunludur')
    return
  }
  if (currentStep.value === 4 && (!form.value.bankIban || form.value.categories.length === 0)) {
    nuxtApp.$toast.warning('IBAN ve en az bir kategori seçmelisiniz')
    return
  }
  if (currentStep.value === 5 && !form.value.agreeTerms) {
    nuxtApp.$toast.warning('Devam etmek için şartları kabul etmelisiniz')
    return
  }
  currentStep.value++
...
const submitForm = async () => {
  loading.value = true
  try {
    const response = await $api('/api/vendors/apply-atomic', {
      method: 'POST',
      body: form.value
    })

    if (response.success) {
      nuxtApp.$toast.success('✅ Başvurunuz başarıyla alındı! Onay bekleniyor.')
      await authStore.init() // Profil durumunu güncelle
      currentStep.value = 6 // Tamamla adımına git veya refresh ile status UI'ını tetikle
    } else {
      nuxtApp.$toast.error(response.error || 'Başvuru sırasında hata oluştu')
    }
  } catch (error) {
    console.error('Registration submit error:', error)
    nuxtApp.$toast.error(error.data?.error || error.data?.message || 'Bir hata oluştu')
  } finally {
    loading.value = false
  }
}
