            ...
            <div class="lg:sticky lg:top-32 space-y-8">
              ...
              <div class="hidden xl:grid grid-cols-3 gap-4 h-32">
                <div class="bg-slate-50 rounded-[2rem] p-6 flex flex-col justify-center border border-slate-100 group hover:bg-white hover:shadow-xl transition-all">
                  <span class="text-xs font-black text-gray-400 uppercase tracking-widest block">{{ product.category?.name || 'Ürün' }}</span>
...
const { data: productData, pending: ssrPending } = await useAsyncData(
  `product-${slugParam.value}`,
  () => productService.getProductBySlug(slugParam.value)
)
...
// Sync SSR data to composable state
