  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (Array.isArray(data.images) && data.images.length > 0) {
        normalizedImages = data.images.map((img: string) => {
      ...
      opportunity.value = {
        id: data.id,
        title: data.title,
        code: `TRX-${data.id.substring(0, 5).toUpperCase()}`,
        postedAt: new Date(data.createdAt).toLocaleDateString('tr-TR'),
        value: `${data.unitPrice || 0}`,
        partner: data.company?.name || 'Kurumsal Satıcı',
        partnerTrust: 95,
        description: data.description || 'Açıklama belirtilmemiş.',
        specs: [
          { label: 'Kategori', value: data.category || '-' },
          { label: 'Miktar', value: `${data.quantity || 0} ${data.unit || 'Birim'}` },
          { label: 'Şehir', value: data.city || '-' },
          { label: 'Statü', value: data.status || '-' }
        ],
        ...
        <div class="col-span-12 lg:col-span-8 space-y-8">
          ...
          <div class="bg-white p-8 rounded-3xl shadow-sm border border-slate-100">
            <div class="flex items-center gap-3 mb-4">
              <span class="px-3 py-1 bg-green-100 text-green-700 rounded-full text-[10px] font-black uppercase tracking-wider italic">AKTİF FIRSAT</span>
          ...
          <div class="bg-white rounded-3xl shadow-sm overflow-hidden border border-slate-100">
            <div class="aspect-video w-full bg-slate-100 relative group">
              <img :src="opportunity.images[0]" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" @error="(e: any) => e.target.src = '/placeholder-image.jpg'" />
            ...
            <div class="p-4 flex gap-4 overflow-x-auto no-scrollbar">
              <div v-for="(img, idx) in opportunity.images" :key="idx" class="w-24 h-24 shrink-0 rounded-2xl overflow-hidden border-2" :class="idx === 0 ? 'border-blue-500' : 'border-transparent'">
                <img :src="img" class="w-full h-full object-cover" @error="(e: any) => e.target.src = '/placeholder-image.jpg'" />
              </div>
            </div>
          </div>
            ...
            <div class="grid md:grid-cols-2 gap-10">
              ...
              <div class="bg-blue-50/50 p-8 rounded-3xl border border-blue-100/50">
                <div class="flex items-center gap-3 mb-4">
                  ...
                  <span class="font-black text-blue-800 text-sm uppercase tracking-tight">Kurumsal Güvence</span>
                </div>
                <p class="text-xs text-slate-500 leading-relaxed">Bu ilan BazarX Kurumsal Denetim ekibi tarafından ön incelemeden geçirilmiştir. Takas işlemleri Escrow (Güvenli Havuz) sistemi ile korunmaktadır.</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="col-span-12 lg:col-span-4 space-y-8">
          ...
          <div class="bg-[#002444] rounded-[2.5rem] p-10 text-white shadow-2xl relative overflow-hidden group">
            ...
            <div class="relative z-10">
              ...
              <div class="space-y-4 mb-10 bg-white/5 p-6 rounded-2xl border border-white/5">
                <div class="flex justify-between text-xs items-center">
                  ...
                  <div class="text-right">
                    <span class="line-through opacity-30 mr-2 text-[10px]">8%</span>
                    <span class="text-green-400 font-black">4% (XP İndirimi)</span>
                  </div>
                </div>
              </div>
              ...
              <div class="space-y-4">
                ...
                <button class="w-full py-5 bg-white/5 hover:bg-white/10 text-white rounded-2xl font-black text-xs uppercase tracking-[0.2em] transition-all border border-white/10 active:scale-95">SATICIYA MESAJ GÖNDER</button>
              </div>
              <p class="text-[9px] text-white/20 mt-8 text-center uppercase tracking-[0.3em] font-bold">Secure Trade Protocol v2.1</p>
            </div>
          </div>
          ...
          <div class="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden">
            <div class="px-8 py-6 bg-slate-50 border-b border-slate-100 flex justify-between items-center">
              ...
              <span class="px-3 py-1 bg-amber-100 text-amber-700 rounded-lg text-[9px] font-black uppercase tracking-widest">GOLD MEMBER</span>
            </div>
            <div class="p-8">
              <div class="flex items-center gap-4 mb-8">
                <div class="w-14 h-14 bg-[#002444] rounded-2xl flex items-center justify-center text-white font-black text-xl shadow-lg">
                  {{ opportunity.partner.charAt(0).toUpperCase() }}
                </div>
                <div>
                  <h4 class="font-black text-[#002444] flex items-center gap-2">
                    {{ opportunity.partner }}
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" class="w-4 h-4 text-blue-500"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"></path></svg>
                  </h4>
                  <span class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Doğrulanmış Satıcı</span>
                </div>
              </div>
              <div class="grid grid-cols-2 gap-4 mb-8">
                <div class="bg-slate-50 p-4 rounded-2xl border border-slate-100 text-center">
                  <span class="block text-[8px] font-black text-slate-400 uppercase mb-1">TRUSTSCORE</span>
                  <span class="text-xl font-black text-green-600">{{ opportunity.partnerTrust }}<span class="text-xs text-slate-400">/100</span></span>
                </div>
                <div class="bg-slate-50 p-4 rounded-2xl border border-slate-100 text-center">
                  <span class="block text-[8px] font-black text-slate-400 uppercase mb-1">DURUM</span>
                  <span class="text-[10px] font-black text-[#002444] uppercase tracking-widest">GÜVENİLİR</span>
                </div>
              </div>
              <button class="w-full py-4 border-2 border-slate-100 hover:border-blue-500 hover:text-blue-600 text-slate-400 rounded-2xl text-xs font-black uppercase tracking-widest transition-all">PROFİLİ İNCELE</button>
            </div>
          </div>
        </div>
      </div>
