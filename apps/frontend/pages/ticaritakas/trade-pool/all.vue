  ...
  <div class="min-h-screen bg-[#F8FAFC]">
    ...
    <main class="max-w-7xl mx-auto px-6 py-12">
      ...
      <div class="mb-12 space-y-8">
        <div class="flex flex-col md:flex-row gap-4">
          <div class="flex-1 relative group">
            <Icon name="heroicons:magnifying-glass" size="20" class="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-blue-500 transition-colors" />
            <input 
              v-model="searchQuery"
              type="text" 
              placeholder="Ürün, hizmet veya firma ara..."
              class="w-full pl-12 pr-6 py-4 bg-white border border-slate-100 rounded-2xl shadow-sm focus:ring-4 focus:ring-blue-500/5 focus:border-blue-500 transition-all font-medium"
              @input="debouncedSearch"
            >
          </div>
          <div class="flex gap-4">
            <select v-model="selectedCity" class="px-6 py-4 bg-white border border-slate-100 rounded-2xl shadow-sm focus:ring-4 focus:ring-blue-500/5 focus:border-blue-500 transition-all font-bold text-xs uppercase tracking-widest">
        ...
        <div class="flex gap-3 overflow-x-auto pb-4 no-scrollbar">
          <button 
            ...
            :class="!selectedCategory ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'bg-white text-slate-500 hover:bg-slate-50'"
            class="px-6 py-2.5 rounded-full text-[10px] font-black uppercase tracking-widest whitespace-nowrap transition-all border border-transparent"
          >
            TÜMÜ
          </button>
          <button 
            ...
            :class="selectedCategory === cat.id ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'bg-white text-slate-500 hover:bg-slate-50 border-slate-100'"
            class="px-6 py-2.5 rounded-full text-[10px] font-black uppercase tracking-widest whitespace-nowrap transition-all border"
          >
            {{ cat.name }}
          </button>
        </div>
      ...
      <div v-if="loading" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
        <div v-for="i in 8" :key="i" class="bg-white rounded-[2rem] h-[420px] animate-pulse border border-slate-50" />
      </div>

      <div v-else-if="items.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
        <div 
          v-for="item in items" 
          :key="item.id"
