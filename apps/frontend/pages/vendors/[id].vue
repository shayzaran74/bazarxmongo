...
const fetchProducts = async (params: Record<string, unknown> = {}, append = false) => {
  ...
  try {
    ...
    if (response.success && response.data) {
      // Use items if it is a PaginatedResponse, else default to array fallback
      const fetchedItems = (response.data as any).items || (response.data as any).data || response.data || [];
      console.log('Fetched products:', fetchedItems.length, 'items', 'Total:', response.pagination?.total)

      if (append) {
        products.value = [...products.value, ...fetchedItems]
      } else {
        products.value = fetchedItems
      }
