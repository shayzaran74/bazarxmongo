        ...
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            ...
            >
                  ...
                  <div class="flex-shrink-0 h-10 w-10">
                    ...
                    >
                      <span class="text-sm font-medium text-gray-700">
                        {{ getUserFullName(user)?.charAt(0) || user.email?.charAt(0) }}
                      </span>
                    </div>
                  </div>
                  <div class="ml-4">
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <span
                  :class="STATUS_CONFIG[user.status?.toUpperCase()]?.class || 'bg-gray-100 text-gray-800'"
                  class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full"
                >
                  {{ STATUS_CONFIG[user.status?.toUpperCase()]?.text || user.status }}
                </span>
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                <span
                  :class="ROLE_CONFIG[user.computedRole]?.class || 'bg-gray-100 text-gray-800'"
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                <span
                  v-if="user.walletTier"
                  :class="TIER_CONFIG[user.walletTier]?.class || 'bg-gray-100 text-gray-800'"
                  class="px-2 py-1 text-xs font-semibold rounded-full"
                >
                  {{ user.walletTier }}
                </span>
                <span
                  v-else-if="user.loyaltyTier"
                  :class="LOYALTY_TIER_CONFIG[user.loyaltyTier]?.class || 'bg-gray-50 text-gray-600 border border-gray-200'"
                  class="px-2 py-1 text-xs font-semibold rounded-full"
                >
                  {{ user.loyaltyTier }}
                </span>
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm">
                <span
                  v-if="user.vendor"
                  :class="VENDOR_STATUS_CONFIG[user.vendor.status]?.class || 'bg-gray-100 text-gray-800'"
                  class="px-2 py-1 text-xs font-medium rounded-full"
                >
                  {{ VENDOR_STATUS_CONFIG[user.vendor.status]?.text || user.vendor.status }}
                </span>
                <span
                  v-else
                  class="text-gray-400 text-xs"
              ...
              <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <div class="flex justify-end space-x-2">
                  ...
                  <button
                    :class="user.status?.toUpperCase() === 'ACTIVE' ? 'text-red-600 hover:text-red-900' : 'text-green-600 hover:text-green-900'"
                    class="text-xs"
                    @click="toggleUserStatus(user)"
                  >
                    {{ user.status?.toUpperCase() === 'ACTIVE' ? 'Pasif Yap' : 'Aktif Yap' }}
                  </button>
                  <button
        ...
        <div class="bg-white rounded-lg shadow-xl max-w-md w-full">
          <div class="bg-gray-50 px-6 py-4 flex justify-between items-center border-b">
            <h2 class="text-lg font-semibold">
              {{ getUserFullName(selectedUser) }} - Satıcı Onayı
            </h2>
            <button
...
const getUserAvatar = (user) => {
  ...
  return avatar.startsWith('http') ? avatar : `${config.public.apiBase}${avatar}`
}
...
const deleteUser = async (user) => {
  if (confirm(`${getUserFullName(user)} kullanıcısını silmek istediğinizden emin misiniz?`)) {
    try {
...
// Configuration Maps for Statuses, Tiers, and Roles
const STATUS_CONFIG = {
  ACTIVE: { class: 'bg-green-100 text-green-800', text: 'Aktif' },
  INACTIVE: { class: 'bg-gray-100 text-gray-800', text: 'Pasif' },
  SUSPENDED: { class: 'bg-yellow-100 text-yellow-800', text: 'Askıya Alınmış' },
  BANNED: { class: 'bg-red-100 text-red-800', text: 'Banlanmış' }
}

const TIER_CONFIG = {
  CORE: { class: 'bg-green-100 text-green-800' },
  PRIME: { class: 'bg-blue-100 text-blue-800' },
  ELITE: { class: 'bg-orange-100 text-orange-800' },
  APEX: { class: 'bg-purple-100 text-purple-800' }
}

const LOYALTY_TIER_CONFIG = {
  BEGINNER: { class: 'bg-emerald-50 text-emerald-700 border border-emerald-200' },
  EXPLORER: { class: 'bg-cyan-50 text-cyan-700 border border-cyan-200' },
  DEDICATED: { class: 'bg-indigo-50 text-indigo-700 border border-indigo-200' },
  VIP: { class: 'bg-amber-50 text-amber-700 border border-amber-200' },
  LEGEND: { class: 'bg-rose-50 text-rose-700 border border-rose-200' }
}

const ROLE_CONFIG = {
  Admin: { class: 'bg-purple-100 text-purple-800' },
  Satıcı: { class: 'bg-blue-100 text-blue-800' },
  Müşteri: { class: 'bg-gray-100 text-gray-800' }
}

const VENDOR_STATUS_CONFIG = {
  APPROVED: { class: 'bg-green-100 text-green-800', text: '✓ Onaylı' },
  PENDING: { class: 'bg-yellow-100 text-yellow-800', text: '⌛ Beklemede' },
  REJECTED: { class: 'bg-red-100 text-red-800', text: '✗ Reddedildi' }
}
...
const approveVendor = async () => {
  ...
  try {
    ...
    useNuxtApp().$toast.success(`✅ ${getUserFullName(selectedUser.value)} onaylandı`)
    closeVendorModal()
    await fetchUsers()
  } catch (error) {
...
const rejectVendor = async () => {
  ...
  try {
    ...
    useNuxtApp().$toast.success(`✗ ${getUserFullName(selectedUser.value)} reddedildi`)
    closeVendorModal()
    await fetchUsers()
  } catch (error) {
    console.error('Hata:', error)
