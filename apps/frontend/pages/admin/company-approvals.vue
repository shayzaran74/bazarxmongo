        ...
        <table class="w-full">
          <thead class="bg-gray-50 border-b border-gray-100">
            <tr>
              ...
              <th class="px-6 py-4 text-left text-[10px] font-black text-gray-400 uppercase tracking-widest">Vergi No</th>
              <th class="px-6 py-4 text-left text-[10px] font-black text-gray-400 uppercase tracking-widest">Konum</th>
              <th class="px-6 py-4 text-left text-[10px] font-black text-gray-400 uppercase tracking-widest">Yetkili</th>
            ...
            >
              <td class="px-6 py-4">
                <div class="flex items-center space-x-3">
                  <div class="w-10 h-10 bg-gray-100 rounded-lg flex items-center justify-center text-xs font-black text-gray-500">
                    {{ item.name.charAt(0) }}
                  </div>
                  <div>
                    <div class="flex items-center gap-2">
                      <div class="font-bold text-gray-900 text-sm">{{ item.name }}</div>
                      <span v-if="item.vendor" class="px-2 py-0.5 bg-blue-50 text-blue-600 rounded text-[9px] font-black uppercase">SATICI</span>
                    </div>
                    <div class="text-xs text-gray-500">{{ item.phone || '-' }}</div>
                  </div>
                </div>
              </td>
              <td class="px-6 py-4 text-sm font-bold text-gray-700">{{ item.taxNumber || '-' }}</td>
              <td class="px-6 py-4">
                <div class="font-bold text-gray-900 text-xs">{{ item.vendor?.user?.profile?.city || '-' }}</div>
                <div class="text-[10px] text-gray-500">{{ item.vendor?.user?.profile?.district || '-' }}</div>
              </td>
...
const fetchPendingCompanies = async () => {
  loading.value = true
  try {
    const response = await $api<{ success: boolean; companies?: CompanyItem[] }>('/api/companies/pending')
    if (response.success) {
...
const approveCompany = async (id: string) => {
  ...
  processing.value = id
  try {
    const response = await $api<{ success: boolean }>(`/api/companies/${id}/status`, {
      method: 'PATCH',
...
const rejectCompany = async (id: string) => {
  const rejectionReason = prompt('Reddetme sebebi (zorunlu değil):') ?? undefined
