<template>
  <div class="py-12 px-6 max-w-4xl mx-auto">
    <div class="flex items-center justify-between mb-12">
      <div>
        <h1 class="text-4xl font-black text-gray-900 italic tracking-tight uppercase leading-none mb-2">
          🔄 BARTERBORSA <span class="text-indigo-600">AYARLARI</span>
        </h1>
        <p class="text-sm font-bold text-gray-400 uppercase tracking-widest opacity-70">BarterBorsa ekosistemi için özel yapılandırma</p>
      </div>
      <button :disabled="saving" class="bg-indigo-600 text-white px-10 py-5 rounded-[24px] font-black uppercase text-[10px] tracking-[0.2em] shadow-xl hover:bg-gray-900 transition-all active:scale-95 flex items-center gap-3" @click="saveSettings">
        ...
        {{ saving ? 'KAYDEDİLİYoR...' : 'BARTERBORSA KAYDET' }}
      </button>
    </div>
    ...
    <VisibilityManager v-model="settings" ecosystem="BARTERBORSA" @toggle="k => settings[k] = !settings[k]" />
...
useHead({ title: 'BarterBorsa Ayarları - Admin' })

const { settings, shippingTiers, saving, logoPreview, logoFile, getLogoUrl, fetchSettings, saveSettings } = useHomepageSettings('BARTERBORSA')
