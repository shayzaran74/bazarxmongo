  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(avatarUrl)) {
        ...
        _push(`<span>${ssrInterpolate(unref(user)?.name?.charAt(0).toUpperCase() || "U")}</span>`);
      }
      _push(`</div><div class="text-center md:text-left space-y-2"><h1 class="text-4xl font-display font-black text-dark-950 italic tracking-tighter">${ssrInterpolate(unref(fullName) || "Kullanıcı")}</h1><p class="text-gray-400 font-medium">${ssrInterpolate(unref(user)?.email)}</p><div class="flex items-center justify-center md:justify-start gap-4 pt-4"><button class="px-6 py-3 bg-dark-950 text-white rounded-2xl text-xs font-black uppercase tracking-widest hover:bg-primary-600 transition-all shadow-lg active:scale-95"> Profili Düzenle </button><button class="px-6 py-3 bg-red-50 text-red-600 rounded-2xl text-xs font-black uppercase tracking-widest hover:bg-red-100 transition-all active:scale-95"> Çıkış Yap </button></div></div></div></div><div class="grid grid-cols-1 md:grid-cols-3 gap-6"><!--[-->`);
      ...
      _push(`<h2 class="text-2xl font-display font-black text-dark-950 italic tracking-tighter">Henüz Bir Aktivite Yok</h2><p class="text-gray-400 text-sm font-medium">Yaptığınız işlemler, teklifler ve favorileriniz burada görünecek.</p>`);
...
//# sourceMappingURL=profile-CipWm9pc.js.map
