...
export const useAdminProducts = () => {
  ...
  const fetchCategoryAttributes = async (categoryId: string | number) => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        categoryAttributes.value.forEach((attr: CategoryAttribute) => {
  ...
  const editProduct = (product: Product & { attributes?: Record<string, unknown>, categoryId?: string | number }) => {
    ...
    if (product.images && Array.isArray(product.images) && product.images.length > 0) {
      formData.value.productImages = product.images.map((img: any): string => 
      ...
      formData.value.productImages = mainImage ? [mainImage as string] : []
    }
