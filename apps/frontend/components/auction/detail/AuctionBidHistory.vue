  ...
  <div class="bg-white rounded-[3.5rem] shadow-2xl shadow-black/[0.02] p-10 lg:p-12 border border-neutral-100 italic">
    ...
    >
      <div
        ...
        :key="bid.id"
        data-testid="bid-row"
        class="flex items-center justify-between p-6 rounded-[2rem] transition-all border group relative"
        :class="index === 0 ? 'bg-indigo-600 border-indigo-200 shadow-2xl shadow-indigo-100 scale-[1.02]' : 'bg-neutral-50 border-transparent hover:bg-white hover:border-neutral-100'"
      >
        <div class="flex items-center gap-6">
          <div
            class="w-12 h-12 rounded-2xl flex items-center justify-center font-black text-xs shadow-inner"
            :class="index === 0 ? 'bg-white text-indigo-600' : 'bg-neutral-200 text-gray-500 group-hover:bg-indigo-50 group-hover:text-indigo-400'"
          >
            #{{ bids.length - index }}
          </div>
          <div>
            <span
              class="block text-sm font-black uppercase tracking-tight"
              :class="index === 0 ? 'text-white' : 'text-gray-900'"
            >{{ maskEmail(bid.User?.email) }}</span>
            <span
              class="text-[9px] font-black uppercase tracking-widest mt-1 opacity-60"
              :class="index === 0 ? 'text-indigo-100' : 'text-gray-400'"
            >{{ formatDate(bid.createdAt) }}</span>
          </div>
        </div>
        <div class="text-right">
          <span
            class="block text-xl font-black tracking-tighter"
            :class="index === 0 ? 'text-white' : 'text-gray-900'"
          >{{ formatPrice(bid.amount) }}</span>
          <span
            v-if="bid.userId === authStore.user?.id"
            class="text-[8px] font-black uppercase tracking-widest leading-none mt-1"
            :class="index === 0 ? 'text-indigo-200' : 'text-indigo-500/60'"
          >TEKLİFİN</span>
        </div>
      </div>
