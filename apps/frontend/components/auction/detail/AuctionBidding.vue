<template>
  <div class="bg-white rounded-[3.5rem] shadow-2xl shadow-black/[0.03] p-10 lg:p-12 border border-neutral-100 relative overflow-hidden italic">
    <div v-if="isHighestBidder" class="absolute top-0 right-0 left-0 h-2 bg-emerald-500 shadow-lg shadow-emerald-500/20" />
    ...
    <div class="space-y-12">
      ...
      <div class="text-center p-10 bg-gray-900 rounded-[3rem] shadow-3xl shadow-gray-900/40 relative overflow-hidden group">
        ...
        <span class="relative z-10 text-5xl sm:text-6xl font-black text-white tracking-tightest leading-none drop-shadow-2xl">{{ formatPrice(auction?.currentPrice || auction?.startingPrice) }}</span>
      </div>
      ...
      <div v-if="auction?.status === 'Active'" class="space-y-8">
        ...
        <div v-if="auction?.participationDeposit > 0 && (!participation || participation.status !== 'Approved')" class="space-y-6">
          <div v-if="!participation" class="bg-amber-50 p-8 rounded-[2.5rem] border border-amber-100/50 text-center space-y-6 shadow-inner">
            ...
            <button class="w-full bg-amber-500 hover:bg-amber-600 text-white py-6 rounded-3xl font-black text-xs uppercase tracking-[0.3em] transition-all shadow-2xl shadow-amber-500/30 active:scale-95" @click="$emit('participate')">
        ...
        <div v-else class="space-y-6">
          <div class="relative group">
            <input v-model.number="bidFieldValue" type="number" :min="minNextBid" class="w-full bg-neutral-50 border-4 border-neutral-100 rounded-[2.5rem] px-10 py-7 font-black text-3xl focus:border-indigo-600 focus:bg-white transition-all outline-none shadow-inner" placeholder="0.00">
          ...
          <button :disabled="loading || bidFieldValue < minNextBid" class="w-full bg-indigo-600 hover:bg-black text-white p-10 rounded-[2.5rem] font-black text-sm uppercase tracking-[0.4em] active:scale-[0.98] transition-all flex items-center justify-center gap-6 shadow-2xl shadow-indigo-100 disabled:opacity-30 group relative overflow-hidden" @click="$emit('place-bid', bidFieldValue)">
      ...
      <div v-if="auction?.status === 'Completed'" class="space-y-8">
         <div class="text-center space-y-8 p-10 bg-indigo-50/50 rounded-[3.5rem] border border-indigo-100/50 shadow-inner">
            ...
            <div v-if="isEligibleToClaim" class="space-y-6 pt-4">
               <button :disabled="claiming" class="w-full bg-emerald-500 hover:bg-emerald-600 text-white p-10 rounded-[2.5rem] font-black text-sm uppercase tracking-[0.4em] shadow-2xl shadow-emerald-100 transition-all active:scale-95 flex items-center justify-center gap-4" @click="$emit('claim')">
