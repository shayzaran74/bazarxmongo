<!-- apps/frontend/components/ticaritakas/SurplusChainModal.vue -->
<template>
          ...
          >
            <DialogPanel class="w-full max-w-2xl transform overflow-hidden rounded-[3rem] bg-white p-12 text-left align-middle shadow-2xl transition-all border border-gray-100">
              ...
              <div v-if="chain" class="space-y-8 italic">
                <div class="flex justify-between items-center bg-neutral-50 p-6 rounded-2xl border border-gray-100 shadow-inner">
                  <div class="flex flex-col">
                    <span class="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-1 leading-none italic">ZİNCİR DURUMU</span>
                    <span :class="getStatusClass(chain.status)" class="text-sm font-black uppercase tracking-widest leading-none">
                      {{ chain.status === 'PENDING' ? 'ONAY BEKLİYOR' : chain.status }}
                    </span>
                  </div>
                ...
                <div class="space-y-6">
                  <div
                    ...
                    :key="offer.id"
                    class="p-8 bg-white border-2 border-dashed border-gray-100 rounded-3xl relative hover:border-indigo-200 transition-colors"
                  >
                    <div class="absolute -left-3 top-1/2 -translate-y-1/2 w-8 h-8 bg-white border-2 border-gray-100 rounded-full flex items-center justify-center text-[10px] font-black text-gray-400 shadow-sm">
                      {{ idx + 1 }}
                    </div>
                    ...
                    <div class="flex justify-between items-start mb-6">
                      <div class="space-y-1">
                        ...
                        <p class="text-base font-black text-gray-900 uppercase italic tracking-tighter">
                ...
                <div v-if="chain.status === 'PENDING' && chain.expiresAt" class="p-6 bg-amber-50 rounded-2xl border border-amber-100 italic flex items-center gap-4">
                  ...
                  <p class="text-[10px] font-black text-amber-700 uppercase tracking-widest leading-loose">
                    BU TAKAS DÖNGÜSÜ İÇİN SON ONAY TARİHİ: {{ formatDate(chain.expiresAt) }}
                  </p>
                </div>
              </div>
              ...
              <div class="mt-12 flex justify-between gap-4 italic uppercase font-black text-[10px] tracking-widest">
                <NuxtLink
                  v-if="chain"
                  to="/my/trades"
                  class="flex-1 h-16 bg-indigo-600 text-white rounded-2xl shadow-2xl shadow-indigo-100 hover:bg-indigo-700 transition-all flex items-center justify-center gap-3"
                >
