<template>
  <div data-testid="checkout-payment" class="bg-white rounded-[2.5rem] shadow-2xl p-8 md:p-10 border border-gray-100 flex flex-col">
    <div class="mb-10 text-center md:text-left">
      <h2 class="text-3xl font-black text-gray-900 mb-2 tracking-tight">Ödeme Bilgileri</h2>
      <p class="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">Güvenli Ödeme Sistemi</p>
    </div>

    <slot name="shipping" />

    <div class="mb-10">
      <h3 class="text-sm font-black text-gray-400 uppercase tracking-widest mb-6">Ödeme Yöntemi</h3>
      ...
      <div class="grid grid-cols-1 gap-3">
        ...
        >
          ...
          >
          <CreditCardIcon class="h-6 w-6 text-gray-400" :class="{ 'text-primary-600': selectedMethod === 'card' }" />
          <div class="flex-1">
            <p class="text-sm font-bold text-gray-900">Kredi/Banka Kartı</p>
            <p class="text-xs text-gray-500">Iyzico ile güvenli ödeme</p>
          </div>
          <!-- Kart logoları -->
          <div class="flex space-x-1 text-xs text-gray-400 font-bold">
            <span class="px-2 py-0.5 bg-gray-100 rounded">VISA</span>
            <span class="px-2 py-0.5 bg-gray-100 rounded">MC</span>
          </div>
        </label>
        ...
        >
          ...
          >
          <WalletIcon class="h-6 w-6 text-gray-400" :class="{ 'text-primary-600': selectedMethod === 'wallet' }" />
          <div class="flex-1">
            <p class="text-sm font-bold text-gray-900">Dijital Cüzdan</p>
            <p v-if="walletLoading" class="text-xs text-gray-400 italic">Yükleniyor...</p>
            <p v-else data-testid="wallet-balance" class="text-xs text-gray-500">Kullanılabilir: {{ formatPrice(walletBalance) }}</p>
          </div>
          <span v-if="!walletLoading && Number(walletBalance) < Number(finalAmount)" class="text-[10px] bg-red-100 text-red-600 px-2 py-0.5 rounded font-bold">
            Yetersiz Bakiye
          </span>
        </label>
      </div>
    ...
    <div v-if="selectedMethod === 'card' && paymentFormContent" class="mb-10">
      <h3 class="text-sm font-black text-gray-400 uppercase tracking-widest mb-6">Ödeme Ekranı</h3>
    ...
    <div v-if="selectedMethod === 'card' && walletBalance > 0" class="mb-6 p-4 bg-primary-50 rounded-xl border border-primary-100">
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-3">
          ...
          <div>
            <p class="text-sm font-bold text-gray-900">Cüzdan Bakiyesini Kullan</p>
            <p data-testid="wallet-balance-mixed" class="text-xs text-gray-500">Mevcut Bakiye: {{ formatPrice(walletBalance) }}</p>
          </div>
        </div>
        <button
        ...
        >
          <span class="inline-block h-4 w-4 transform rounded-full bg-white transition-transform" :class="useWalletBalance ? 'translate-x-6' : 'translate-x-1'" />
        </button>
      </div>
    </div>
    ...
    <div class="mb-6 p-4 bg-gray-50 rounded-xl border border-gray-100">
      <div class="flex items-start gap-3">
        ...
        >
        <label for="accept-terms" class="text-xs text-gray-600 leading-relaxed cursor-pointer select-none">
          <span class="text-primary-600 font-bold hover:underline cursor-pointer" @click.prevent="$emit('openLegal', 'on-bilgilendirme-formu')">Ön Bilgilendirme Koşulları</span>'nı ve
          <span class="text-primary-600 font-bold hover:underline cursor-pointer" @click.prevent="$emit('openLegal', 'mesafeli-satis-sozlesmesi')">Mesafeli Satış Sözleşmesi</span>'ni okudum, onaylıyorum.
        </label>
      </div>
    </div>
    ...
    <div class="space-y-4">
      <button
        ...
        :class="{ 'opacity-50 cursor-not-allowed': processing || !isFormValid }"
        @click="$emit('pay')"
      >
      ...
      <button type="button" class="w-full py-3 text-sm font-bold text-gray-500 hover:text-gray-700 transition-colors" @click="$emit('back')">
        Sepete Dön
      </button>
    </div>
