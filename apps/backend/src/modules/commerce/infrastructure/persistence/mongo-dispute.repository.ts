...
import { DisputeMapper, DisputeDocument, DisputeDto } from './mappers/dispute.mapper';
import { IDisputeRepository, DisputeRecord } from '../../domain/repositories/dispute.repository.interface';
...
export class MongoDisputeRepository implements IDisputeRepository {
  ...
  async list(filters: { status?: string; vendorId?: string; userId?: string }): Promise<DisputeRecord[]> {
  ...
  async save(dispute: Partial<DisputeRecord>): Promise<DisputeRecord> {
    const doc = DisputeMapper.toPersistence(dispute as DisputeDto);
