...
export class GetListingBySlugHandler implements IQueryHandler<GetListingBySlugQuery> {
  private readonly logger = new Logger(GetListingBySlugHandler.name);

  constructor(private readonly anonymizer: AnonymizerService) {}

  async execute(query: GetListingBySlugQuery) {
    this.logger.debug(`execute called with slug: "${query.slug}"`);
    ...
    this.logger.debug(`isId: ${isId}`);

    const listing = await Listing.findOne(isId ? { id: query.slug } : { slug: query.slug }).lean().exec() as ListingLean | null;
    this.logger.debug(`Listing.findOne result exists: ${!!listing}`);
    if (!listing) {
      this.logger.debug(`Listing not found for query slug: "${query.slug}", returning null`);
      return null;
    }
    ...
    const cp = listing.catalogProductId
      ...
      : null;
    this.logger.debug(`CatalogProduct find result exists: ${!!cp}`);
    ...
    if (listing.vendorId) {
      if (isEcosystemListing) {
        ...
        this.logger.debug(`Ecosystem listing, anonymizing vendor: ${anonymousVendorId}`);
      } else {
        vendor = await Vendor.findOne({ id: listing.vendorId }).lean().exec();
        this.logger.debug(`Normal listing, vendor found: ${!!vendor}`);
      }
    }
