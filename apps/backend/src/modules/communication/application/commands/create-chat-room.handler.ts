...
export class CreateChatRoomHandler implements ICommandHandler<CreateChatRoomCommand> {
  private readonly logger = new Logger(CreateChatRoomHandler.name);
  ...
  async execute(command: CreateChatRoomCommand) {
    if (command.orderId) {
      ...
      this.logger.log(`Chat room created for order ${command.orderId}: initiator=${order.userId}, receiver=${order.vendorId}`);
    ...
    if (command.tradeOfferId) {
      ...
      const initiatorId = (trade as Record<string, unknown>).initiatorId as string;
      const receiverId  = (trade as Record<string, unknown>).receiverId as string;
      this.logger.log(`Chat room created for trade ${command.tradeOfferId}: initiator=${initiatorId}, receiver=${receiverId}, commandParticipants=${JSON.stringify(command.participantIds)}`);

      const room = ChatRoom.createForTrade(trade.id, initiatorId, receiverId);
      await this.repository.save(room);
      return { success: true, data: { id: room.id } };
    }
