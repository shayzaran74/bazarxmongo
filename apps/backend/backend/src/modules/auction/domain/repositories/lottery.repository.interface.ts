...
export interface ILotteryRepository extends IRepository<Lottery> {
  findById(id: string, session?: ClientSession): Promise<Lottery | null>;
  ...
  createTicket(ticket: { lotteryId: string; userId: string; numbers: string[] }, session?: ClientSession): Promise<ILotteryTicket>;
  countTickets(lotteryId: string, userId?: string, session?: ClientSession): Promise<number>;
  findTicketWithNumbers(lotteryId: string, numbers: string[], session?: ClientSession): Promise<ILotteryTicket | null>;
