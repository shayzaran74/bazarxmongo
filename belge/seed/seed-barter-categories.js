...
async function main() {
    console.log('🌱 Barter kategorileri ekleniyor...')

    // 1. Hierarchical Categories (Category table & SurplusCategory table)
    async function upsertHierarchicalCategories(categories, parentId = null, surplusParentId = null) {
        for (const cat of categories) {
            ...
            // Surplus Category Table (Now hierarchical too)
    ...
    // 2. Extra flat categories mentioned in Section 3 (Çapraz Takas)
    const extraSectors = [
    ...
    for (const sector of extraSectors) {
        ...
        await prisma.surplusCategory.upsert({
            where: { name: formattedName },
            update: { isActive: true, slug },
            create: { name: formattedName, isActive: true, slug }
        })
    }
