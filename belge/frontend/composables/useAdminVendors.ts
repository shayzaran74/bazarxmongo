...
export const useAdminVendors = () => {
  ...
  const availableCategories = computed(() => {
    ...
    const assignedCategoryIds = selectedVendor.value.categories?.map(c => c.id) || []
  ...
  const addCategory = async (vendorId: string | number) => {
    ...
    try {
      ...
      const newCatEntry: Category = {
        id: `${vendorId}-${selectedCategoryId.value}`,
        name: category?.name || '',
        slug: category?.slug || '',
  ...
  const removeCategory = async (vendorId: string | number, categoryId: string | number) => {
    try {
      ...
      if (selectedVendor.value) {
        selectedVendor.value.categories = selectedVendor.value.categories?.filter(c => c.id !== categoryId)
      }

      const idx = vendors.value.findIndex(v => v.id === vendorId)
      if (idx !== -1) {
         vendors.value[idx].categories = vendors.value[idx].categories?.filter(c => c.id !== categoryId)
      }
