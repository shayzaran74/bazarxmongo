...
export const useProductForm = (props: { productId?: string | null, initialData?: Partial<ProductFormState> }) => {
  ...
  const handleFileUpload = async (event: Event) => {
    ...
    for (let i = 0; i < limit; i++) {
      ...
      try {
        const response = await $api<ApiResponse & { url?: string }> (`/api/upload?type=product&category=${encodeURIComponent(categoryName)}`, {
          method: 'POST',
          body: formData
        })
        if (response.success && response.url) {
          form.value.productImages.push(response.url)
          if (!form.value.image) form.value.image = response.url
        }
  ...
  const fetchCategories = async () => {
    try {
      const res = await $api<ApiResponse<Category[]>>('/api/categories?all=true')
  ...
  const fetchCategoryAttributes = async (catId: string) => {
    ...
    try {
      const res = await $api<ApiResponse<CategoryAttribute[]>>(`/api/categories/${catId}/attributes`)
      if (res.success) categoryAttributes.value = res.data
    } catch (e) { console.error(e) }
  }
  ...
  const checkBarcode = async () => {
    ...
    try {
      const res = await $api<ApiResponse<Product>>(`/api/products/barcode/${form.value.barcode}`)
      if (res.success && res.data) {
        foundCatalogProduct.value = res.data
        toast.success('Katalog eşleşmesi sağlandı.')
        form.value.name = res.data.name
        form.value.description = res.data.description || ''
        form.value.brand = res.data.Brand?.name || (res.data.brand as string) || ''
        
        if (res.data.Category?.id) {
          form.value.categoryId = String(res.data.Category.id)
        }
        
        form.value.modelCode = res.data.sku || ''
        form.value.technicalSpecifications = (res.data as Product & { technicalSpecifications?: Record<string, string | number> }).technicalSpecifications || {}
        
        if (res.data.images && res.data.images.length > 0) {
          form.value.productImages = res.data.images.map((img: string | { url: string }) => 
