...
export const useCheckout = () => {
  ...
  const finalAmountExcludingLoyalty = computed(() => {
    ...
    return Math.max(0, Number(cartStore.totalPrice) + Number(shippingCost.value) - discount - escrowDiscount)
  })
  ...
  const formatDocumentContent = (content: string): string => {
    ...
    const buyerName = (showNewAddressForm.value ? newAddress.value.fullName : selectedAddress?.fullName) || `${authStore.user?.profile?.firstName || ''} ${authStore.user?.profile?.lastName || ''}`.trim() || 'ALICI'
    const buyerPhone = (showNewAddressForm.value ? newAddress.value.phone : selectedAddress?.phone) || authStore.user?.profile?.phone || ''
    ...
    if (showNewAddressForm.value) {
      ...
      buyerAddress = [selectedAddress.addressLine, selectedAddress.district, selectedAddress.city].filter(Boolean).join(', ')
    }
  ...
  const buildPaymentPayload = (): CheckoutPaymentPayload => {
    const addrId = selectedAddressId.value && !showNewAddressForm.value ? selectedAddressId.value : null
  ...
  const applyCoupon = async (code?: string) => {
    ...
    try {
      const coupon = await checkoutService.validateCoupon(codeToUse, cartStore.totalPrice)
  ...
  const initializeStripe = async () => {
    ...
    try {
      ...
      const data = await checkoutService.createPaymentIntent(payload)
  ...
  const handlePayment = async () => {
    ...
    try {
      if (selectedMethod.value === 'card') {
        ...
        // Wallet Payment
        const payload = buildPaymentPayload()
        const data = await checkoutService.processWalletPayment(payload)
        
        await cartStore.fetchCart()
        return { success: true, orderId: data.orderId, type: 'wallet' }
      }
    } catch (error: unknown) {
      const err = error as Error
      return { success: false, error: err.message }
  ...
  const fetchWallet = async () => {
    ...
    try {
      ...
      if (response && response.success) {
        ...
        if (addr.accounts) {
          const mainAccount = addr.accounts.find(a => a.type === 'MAIN')
          if (mainAccount) {
            walletBalance.value = Number(mainAccount.availableBalance)
          }
        }
      }
    } catch (err) {
  ...
  const fetchShippingCost = async () => {
    try {
      const hasPremium = cartStore.items.some(item => (item.Product?.name || '').toLowerCase().includes('premium'))
      if (hasPremium) {
        shippingCost.value = 0
        return
      }

      const settings = await checkoutService.fetchSettings()
      if (settings?.shippingCost) {
        shippingCost.value = parseFloat(settings.shippingCost as string)
      }

      if (settings?.shippingTiers) {
        const tiers = JSON.parse(settings.shippingTiers as string) as Array<{ min?: number, max?: number, cost: number | string }>
        const tier = tiers.find(t => cartStore.totalPrice >= (t.min || 0) && cartStore.totalPrice <= (t.max || 9999999))
        shippingCost.value = tier ? Number(tier.cost) : 0
        return
      }
      
      shippingCost.value = cartStore.totalPrice >= 500 ? 0 : 50
    } catch (err) {
  ...
  const init = async () => {
    ...
    Promise.allSettled([
      checkoutService.fetchLegalDocuments().then(docs => docs.forEach(d => { legalDocs.value[d.slug] = d })),
      checkoutService.fetchLoyaltyStatus().then(status => { loyaltyStatus.value = status }),
    ...
    if (escrowCouponId.value) {
      try {
        ...
        const found = escRes.find(c => c.id === escrowCouponId.value)
  ...
  watch(() => cartStore.totalPrice, () => fetchShippingCost())
