...
export const useAdminOrderDetail = (orderId: string) => {
  ...
  const fetchOrder = async () => {
    loading.value = true
    try {
      const response = await $api<ApiResponse<AdminOrder>>(`/api/admin/orders/${orderId}`)
      if (response.success) {
        order.value = response.data
        shippingUpdate.value = {
          status: order.value.status,
          trackingNumber: order.value.trackingNumber || '',
          shippingCarrier: order.value.shippingCarrier || '',
          estimatedDelivery: order.value.estimatedDelivery ? order.value.estimatedDelivery.split('T')[0] : '',
