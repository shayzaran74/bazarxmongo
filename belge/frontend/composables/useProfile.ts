...
import { userService, type UserProfileUpdate } from '~/services/userService'
import type { Address, LoyaltyStatus, LoyaltyHistoryItem, UserProfileStats, WalletTransaction } from '@barterborsa/shared-types'
  ...
  const uploadAvatar = async () => {
    ...
    try {
      ...
      if (uploadRes.success) {
        const profileRes = await userService.updateProfile({ avatar: uploadRes.url })

        if (profileRes.success && authStore.user) {
          if (authStore.user.profile) {
            authStore.user.profile.avatar = uploadRes.url || null
          }
