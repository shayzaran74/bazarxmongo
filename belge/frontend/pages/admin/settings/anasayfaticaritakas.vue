  ...
  <div class="p-8 max-w-4xl mx-auto">
    ...
    <div class="mb-8">
      <h1 class="text-3xl font-bold text-gray-900">
        🏭 Ticari Takas Ana Sayfa Ayarları
      </h1>
      <p class="text-gray-600 mt-1">
        Ticari Takas ana sayfasında hangi bölümlerin görüneceğini yönetin
      </p>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
      ...
      <div class="p-6 space-y-8">
        ...
        >
          ...
          <div class="flex items-center space-x-4">
            <NuxtLink
              to="/admin/settings/side-ads?ecosystem=TICARITAKAS"
        ...
        >
          ...
          <div class="flex items-center space-x-4">
            <NuxtLink
              to="/admin/banners?ecosystem=TICARITAKAS"
              class="text-xs font-bold text-primary-600 hover:underline px-3 py-1 bg-white rounded-lg border border-gray-200"
            >
              Bannerları Yönet
            </NuxtLink>
...
const fetchSettings = async () => {
  try {
    ...
    if (response.success) {
      const getVal = (key) => response.data[`ticaritakas_${key}`] !== undefined ? response.data[`ticaritakas_${key}`] : response.data[key]
      ...
      booleanKeys.forEach(key => {
        ...
        if (val !== undefined) {
          // Some are stored as strings "true"/"false" in DB settings
          if (val === 'true') settings.value[key] = true
          else if (val === 'false') settings.value[key] = false
          else settings.value[key] = !!val
        }
      })
      ...
      stringKeys.forEach(key => {
        const val = getVal(key)
        if (val !== undefined) settings.value[key] = val
      })

      if (getVal('shippingTiers')) {
        settings.value.shippingTiers = getVal('shippingTiers')
        try {
          shippingTiers.value = JSON.parse(getVal('shippingTiers'))
        } catch (e) {
          shippingTiers.value = []
        }
      }
...
const saveSettings = async () => {
  ...
  try {
    ...
    const body = {}
    const prefix = 'ticaritakas_'

    // Boolean settings to save
    const booleanKeys = [
      'showAuctions', 'showLotteries', 'showGroupBuy', 'showFlashSales',
      'showSpecialOffers', 'showHomeSlider', 'showAds', 'showQuadCards',
      'showBarterPool', 'showPersonalized'
    ]
    // String settings to save
    const stringKeys = [
      'siteName', 'siteLogo', 'adLeftEmoji', 'adLeftTitle', 'adLeftSubtitle',
      'adLeftLink', 'adLeftImage', 'adRightEmoji', 'adRightTitle',
      'adRightSubtitle', 'adRightLink', 'adRightImage',
      'autoApproveListings', 'autoApproveOffers'
    ]

    booleanKeys.forEach(key => {
      body[`${prefix}${key}`] = (settings.value[key] ?? true).toString()
    })
    stringKeys.forEach(key => {
      body[`${prefix}${key}`] = settings.value[key]
    })
    body[`${prefix}shippingTiers`] = JSON.stringify(shippingTiers.value)
