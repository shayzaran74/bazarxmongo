    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      >
        ...
        >
                ...
                <div class="flex -space-x-3 overflow-hidden">
                  <template v-if="order.OrderItem && order.OrderItem.length > 0">
                    <img
                      ...
                      :key="item.id"
                      :src="resolveImageUrl(item.Listing?.CatalogProduct?.images?.[0]) || 'https://placehold.co/100x100?text=Ürün'"
          ...
          <!-- Tracking Info Footer if available -->
          <div
            v-if="order.trackingNumber || order.OrderItem?.some(i => i.trackingNumber)"
            class="bg-gray-50 px-6 py-3 flex items-center justify-between border-t border-gray-100"
          >
            <div class="flex items-center text-xs text-gray-600">
              <TruckIcon class="h-4 w-4 mr-2 text-primary-500" />
              <span v-if="order.trackingNumber">Kargo Takip: <b>{{ order.shippingCarrier }}</b> - <b>{{
                order.trackingNumber
              }}</b></span>
              <span v-else>Ürünlerinizden bazıları kargoya verildi. Detaylar için tıklayın.</span>
            </div>
            <NuxtLink
              :to="`/orders/${order.id}`"
