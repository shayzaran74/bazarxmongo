...
| Tier | Erişilebilir Kategoriler |
|:---|:---|
| Bronz Prime 1 | 6 — Dondurma & Ekler |
| Bronz Prime 2 | 5 — Kahve & İçecek + altı |
| Gümüş Prime 1 | 4 — Tatlı & Pastane + altı |
| Gümüş Prime 2 | 3 — Casual Dining & Yerel + altı |
| Altın Prime 1 | 2 — Mid-Point & New Casual + altı |
| Altın Prime 2 | 2 — Mid-Point & New Casual + altı (genişletilmiş kota) |
| Elmas Prime 1 | 1 — VIP & Fine Dining + tümü |
| Elmas Prime 2 | 1 — VIP & Fine Dining + tümü (öncelikli rezervasyon) |
...
Bonus tier'ı şu şekilde hesaplanır:

```
3 referansın toplam ödediği aidat toplamı
        ↓
Bu toplama en yakın tier bulunur
        ↓
O tier'ın bir altındaki tier'dan 1 adet menü hakkı verilir
```
...
| Faz 4 — Delivery | ✅ | DeliveryDispatch schema, BullMQ queue (Push/Mail sonraki sprint) |
| **Faz 5 — Bildirim** | 🔜 | FCM Push + Mail (Resend/Postmark) entegrasyonu |
| **Faz 6 — Geofencing** | 🔜 | Konum bazlı tetikleyici servisi |
| **Faz 7 — Rezervasyon** | 🔜 | Uygulama içi rezervasyon modülü |
...
- [ ] Altın P1 ve üstü için XP eşik değerleri netleştirilecek (admin paneli konfigürasyonu)
- [ ] Referans bonus algoritması backend'e işlenecek (§7 pseudocode'u baz alarak)
- [ ] Mail sağlayıcı seçimi: Resend mi, Postmark mi? (deliverability karşılaştırması yapılacak)
- [ ] Geofencing yarıçapı kullanıcı testine açılacak (500m başlangıç, ayarlanabilir)
- [ ] Ankara pilot için ilk 20 restoran listesi çıkarılacak
- [ ] Sürpriz menü özelliği için A/B test planı yapılacak
