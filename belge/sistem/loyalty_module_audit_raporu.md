...
| # | Dosya | Düzeltme |
|---|-------|----------|
| 1 | `xp-rules.service.ts:76` | `erodeExpiredBatches()` — cron job ekle veya başka servis çağır |
| 2 | `missions-milestones.entities.ts:36,46` | `MissionProgress as any` kaldır — tip güvenliği |

### Sonraki Sprint (YÜKSEK)

| # | Dosya | Düzeltme |
|---|-------|----------|
| 3 | `xp-management.handlers.ts:28` | `checkFirstOrderBlock()` çağır — ilk işlem kuralı |
| 4 | `xp-rules.service.ts:76` | Redis distributed lock ekle — multi-instance koruması |

### Backlog (ORTA)

| # | Dosya | Düzeltme |
|---|-------|----------|
| 5 | `xp-rules.service.ts:9-16` | `XP_EARNING_RULES` → `XpDistributionRule` DB'den oku (admin CRUD etkinleştir) |
| 6 | `mongo-loyalty.repositories.ts:119,124` | `raw as any` → tip güvenli mapper |
...
| 9 | MilestoneChecker + TierController | Redis cache invalidation doğrula |

### Belgeleme (DÜŞÜK)

| # | Dosya | Not |
|---|-------|-----|
| 10 | `PlatinumMissionLog` | Kullanımda mı, yazılıyor mu doğrula |
...
Loyalty modülü **yapısal olarak sağlam** — DDD entity/value object yapısı doğru, mission/milestone entity'leri mevcut, spending limit service tip güvenliği sağlıyor. `TierRateLimitGuard` ve üç tier sistemi ayrı tutulmuş.

**En kritik sorunlar:**

1. **`erodeExpiredBatches()` hiç çalışmıyor** — method mevcut ama cron/scheduler tetiklemiyor. XP 6 ay geçerlilik kuralı dormant. Bu düzeltilmezse sistemde XP birikir, erime işlemi hiçbir zaman olmaz.

2. **`MissionProgress as any`** — Mission entity'sinde Value Object tipi `as any` ile bypass ediliyor. Bu düzeltilmezse mission progress hesabı runtime'da yanlış olabilir.
