...
## 3.5 Platform Operasyon Kuralları

### 3.5.1 İade ve Cayma Hakkı

- Müşteri, ürün tesliminden itibaren **14 gün** içinde hiçbir gerekçe göstermeksizin iade açabilir (Mesafeli Satış Sözleşmeleri Yönetmeliği zorunluluğu).
- İade talebi açıldığında platform otomatik kargo kodu üretir; iade kargosu **satıcı tarafından karşılanır**.
- Satıcı iade talebini **48 saat** içinde onaylamalı veya gerekçeli olarak reddetmelidir. Süre aşımında iade otomatik onaylanır.
- Satıcı gerekçesiz veya mevzuata aykırı red yaparsa sipariş tutarı satıcıdan tahsil edilir.
- Onaylanan iadelerde geri ödeme **3 iş günü** içinde müşteriye yapılır.
- **İade Gelir Modeli:** Platform iade kargo bedelini satıcı hakedişinden keser; ayrıca işlem başına sabit **5 ₺ iade işlem bedeli** alır.

### 3.5.2 Satıcı Puanlama ve İhlal Sistemi

Satıcılar aşağıdaki 4 bileşenden oluşan **Mağaza Puanı** ile değerlendirilir (0–100):

| Kriter | Ağırlık |
|:---|:---:|
| Zamanında kargo | %35 |
| Satıcı kaynaklı iptal oranı | %25 |
| Satıcı kusurlu iade oranı | %20 |
| Müşteri yorum ortalaması | %20 |

**Puan Kademeleri ve Aksiyon:**

| Puan | Durum | Platform Aksiyonu |
|:---|:---:|:---|
| 80–100 | İyi | Normal |
| 60–79 | Uyarı | Satıcıya bildirim |
| 40–59 | Kritik | Yeni ürün ekleme kısıtlanır |
| 0–39 | Askıya alındı | Satış durdurulur, admin incelemesi |

- Her ihlal kararına karşı **3 iş günü** itiraz hakkı tanınır.
- Satıcı puanı algoritma kataloğunda görünürlüğü doğrudan etkiler.

### 3.5.3 Buybox Mekanizması

Aynı ürünü birden fazla satıcı listelediğinde **Buybox skoru** en yüksek satıcı ürün sayfasının ana "Sepete Ekle" butonuna atanır. Diğer satıcılar "X satıcıdan daha ucuzu" bölümünde listelenir.

**Buybox Skoru Bileşenleri:**

| Kriter | Ağırlık |
|:---|:---:|
| En düşük toplam fiyat (ürün + kargo) | %40 |
| Satıcı mağaza puanı | %30 |
| Ortalama kargoya verme süresi | %20 |
| Stok güvenilirliği (son 30 gün) | %10 |

### 3.5.4 Satıcı Erken Ödeme

- Satıcı onaylı siparişlerin hakedişinin **en fazla %80'ini** vade öncesi çekebilir.
- Erken ödeme bedeli: kalan vade günü × günlük **%0.05** (platform tarafından güncellenir).
- Minimum tutar: **500 ₺** — günde **1** talep hakkı.
- **Gelir Modeli:** Erken ödeme faizi platform geliridir; finansal servisten anlık transfer yapılır.

### 3.5.5 Otomatik E-Fatura

- Sipariş teslim edildiğinde GİB uyumlu **e-arşiv fatura** otomatik kesilir.
- Fatura müşteriye e-posta ve uygulama bildirimiyle iletilir.
- Satıcı kendi faturalarını platformdan indirebilir.
- Entegratör: platform tarafından belirlenir (efatura.com / Logo / Paraşüt); satıcıdan ek aksiyon gerekmez.
